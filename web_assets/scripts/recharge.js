$(document).ready(function() {
    ...
    function operators() {
        var ur = baseurl + 'APIS/recharge_api/get_operators/1';
        $.ajax({
            url: ur,
            success: function(res) {
                var obj = JSON.parse(res);
                if (obj.err_code == 1) {
                    ...
                    for (var i = 0; i < obj.message.length; i++) {
                        $('.operators').append('<option test value="' + obj.message[i].operator_code + '">' + obj.message[i].operator_name + '</option>');
                    }
    ...
    operators();
    function operators2() {
        var ur = baseurl + 'APIS/recharge_api/get_operators/2';
        $.ajax({
            url: ur,
            success: function(res) {
                var obj = JSON.parse(res);
                if (obj.err_code == 1) {
                    $('.operators2').html('<option value="">Select Operator</option>');
                    for (var i = 0; i < obj.message.length; i++) {
                        $('.operators2').append('<option value="' + obj.message[i].operator_code + '">' + obj.message[i].operator_name + '</option>');
                    }
                } else $('.operators2').html('<option>No operators</option>')
            },
            error: function(res) {}
        })
    }
