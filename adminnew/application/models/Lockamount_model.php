 ...
 */
class lockamount_model extends CI_Model {
	...
	public function get_lockamount($id){
		
		$this->db->select('locaking_amount.*');

		$this->db->from('locaking_amount');
	...
	public function update_lockamttable($data,$id){

			$this->db->where('id',$id);
			$return = $this->db->update('locaking_amount', $data); 
					// return true;
