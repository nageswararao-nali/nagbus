...
class offer_model extends CI_Model {
	...
	public function update_offertable($data,$id){
		...
		public function update_agentsubwalletoffertableprodnew($userData) {				
		if ($this->db->insert("agent_subscription_offers", $userData)) {
				return $this->db->insert_id();
			} else {
				return FALSE;
			}		
		...
		public function get_agent_sub_offer()
		{
			$query = $this->db->query("SELECT * from agent_subscription_offers order by id desc  ");				
			return $query->result_array();
		}
		
		public function get_agent_sub_status()
		{
			$query = $this->db->query("SELECT * from agent_subscription_status order by id desc  ");				
			return $query->result_array();
		}
		
		public function update_agent_sub_status($data,$id=''){
			$this->db->where('id',1);
			$return = $this->db->update('agent_subscription_status', $data); 
			return true;			
		}
