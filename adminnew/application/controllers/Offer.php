...
class Offer extends CI_Controller {
	...
	function update_agent_sub_status()
	{
		$data["status"] = $_REQUEST['status'];
		$this->offer_model->update_agent_sub_status($data);
		echo 1;
		exit;
	}
	...
	function Edit_subscription(){
		...
		$data['offers'] = $this->offer_model->get_agent_sub_offer();

		$data['aget_sub_status'] = $this->offer_model->get_agent_sub_status();
		
		//print_r($data['aget_sub_status']);

		$this->load->view('admin_template/Header');
		
		$this->load->view('offer/Edit_subscription',$data);
	...
	public function delete_agent_sub_offer()
	{
		$this->db->query('delete from agent_subscription_offers where id = ' . $_REQUEST["id"]);
		echo 1;
		exit;
	}
	
	public function delete_users_ByAdmin()
	{
		$this->db->query('delete from users where user_id = ' . $_REQUEST["user_id"]);
		echo 1;
		exit;
	}
	...
	function update_walletofferamountnew(){
		...
		foreach($_REQUEST['org_amount'] as $key => $value)
		{
			...
			$user_data['org_amount_max'] = $_REQUEST['org_amount_max'][$key];
		...
		function update_agentofferamountnew(){
		...
		$description = $this->input->post('description');
			...
			$user_data['subscription_amount'] = $this->input->post('subscription_amount');
			if($this->input->post('offer_mode') == "INR")
			{
				$user_data['wallet_amount'] = $this->input->post('wallet_amount');
				$user_data['admin_amount'] = $this->input->post('subscription_amount') - $this->input->post('wallet_amount');
			}
			else
			{
				$user_data['percentage_applicable'] = $this->input->post('wallet_amount');
				$user_data['wallet_amount'] = $this->input->post('subscription_amount')*$this->input->post('wallet_amount')/100;
				$user_data['admin_amount'] = $this->input->post('subscription_amount') - $user_data['wallet_amount'];
				
			}
			
			
			$user_data['offer_mode'] = $this->input->post('offer_mode');						
			$user_data['start_date'] = $st_date;
			$user_data['end_date'] = $end_date;			
			$user_data['promo_code']= $promo_code;
			$user_data['description']= $description;
			$this->offer_model->update_agentsubwalletoffertableprodnew($user_data);
