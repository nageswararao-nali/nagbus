...
class Users_Model extends CI_Model {
	...
	public function get_AgentCommisionAmountBySubCat($subcatid) {
		$this->db->select('agent_comm_value,agent_ref_comm_value');
...
/*****************************************************************************************************************/
	...
	public function mobile_exists($role_id,$mobile) {
		...
		{
			$date = 'Y-m-d';
			//$result = $this->db->query("SELECT SUM(agent_comm+agent_ref_comm) as earnings FROM `transaction` WHERE transaction_status = 1 and (`end_user_id`= $user_id OR `agent_id`= $user_id)  and date(order_date) BETWEEN '2016-05-01' AND '2016-05-31'  " )->result_array();
			$result = $this->db->query("SELECT SUM(agent_comm+agent_ref_comm) as earnings FROM `transaction` WHERE 1 = 1 and (`end_user_id`= $user_id OR `agent_id`= $user_id)  and date(order_date) BETWEEN '2016-06-01' AND '2016-06-30'  " )->result_array();
            //echo $this->db->last_query();exit;
            return $result;
		}
