...
class User extends CI_Controller {
	...
	public function printTicket($tin){
        ...
        echo "<pre>";
        print_r(json_decode($ticket));
        echo "</pre>";
        
    }
    
    
    public function PassengerList($bookingKey){     
...
$tot_W_a = $wallet_amount+$totamount;
  ...
  $message = 'Dear '.$name.', Your Wallet is filled with INR '.$totamount.'. Your net Wallet Amount is '.$tot_W_a.'. thank you for using w LAABUS . download  app @ https://goo.gl/QWUiJ';
	...
	public function Orders() {
		...
		//$data1['order_details'] = $this->Va_Commisions_model->order_details_user_new($search_params);
		
		echo $email = $this->session->userdata('email_id');
		
		$data1['ticket_details'] = $this->ticket->get_ticketDetials($email);
