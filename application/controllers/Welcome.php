...
//include_once '/seatseller/BuServiceList.php';
	...
	public function register() {
		...
		$this->form_validation->set_rules('mobile', 'Mobile', 'trim|required|min_length[10]|callback_check_mobile_role_exists|is_unique[login.Mobile]');
...
public function e_com() {
    ...
    public function check_email_role_exists($password) {
	 ...
	 public function check_mobile_role_exists($password) {
        ...
        $role_id = $this->input->post('usertype');
        $emailcount=$this->users->mobile_exists($role_id,$mobile);
         if($emailcount->cnt==0){
                return true;
            }else{
	...
	public function subscriptionaction()
	{
		...
		{
			$sales_id = $_REQUEST["txnid"].'&amount='.$_REQUEST["amount"];
			redirect('Payment/agent_subscription_payu?txnid='.$sales_id);
		}
		else
		{
			$this->session->set_flashdata('msg', 'Registraion Successfull Please Login');
