	...
	function index(){
		...
		$data['roles'] = $this->users->get_roles();
                if(!$this->input->is_ajax_request()) {
                    ...
                    $this->load->view('website/privacy/index');
                    $this->load->view('website_template/footer');
                } else {
                    $this->load->view('website/privacy/index', $data);
                }
	}
	
	function moreoffers(){
            //print_r($this->session->userdata());
			$data['offers1'] = $this->users->getoffers(6);
		$data['offerswallet1'] = $this->users->getofferswallet(6);
		$data['category'] = $this->Cat->get_category();
		
		$data['offers2'] = $this->users->getoffersusers(4);
		$data['offerswallet2'] = $this->users->getofferswalletusers(4);
		
		$data['offers3'] = $this->users->getoffersusers(44);
		$data['offerswallet3'] = $this->users->getofferswalletusers(44);
