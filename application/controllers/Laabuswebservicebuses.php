...
//include_once 'seatseller/index.php'; //comment by Venkat.
//include_once '/seatseller/BuServiceList.php';
require_once APPPATH.'controllers/Template.php';

class Laabuswebservicebuses extends Template {
   ...
   function __construct() {
    ...
    {
    	$cancellationId = $_REQUEST['ticket_no'];
    	    $ticket = array();
            //$ticket['tin']= $_REQUEST['tin']; 
            $ticket['tin']= $_REQUEST['ticket_no']; 
            $ticket['seatsToCancel'] = $_REQUEST['seatName'];
            
            $json = json_encode($ticket);
            
           $data1 =  cancelTicket($json);
           
    	$response['message'] = getCancellationData($cancellationId);
    	header('Content-Type: application/json');       
       echo json_encode($response);
    }
	
	function blockTicketLaabusTest()
	{
		$jsonVar = $_REQUEST["booking"];
		//echo $jsonVar;
		$onword = json_decode($jsonVar);
		//print("<pre>");
		//print_r($json_decode_data);
		$bookingKey = uniqid();
	 if($this->Booking_model->create_booking_app($bookingKey,$onword)){
	 	echo "Okay";
	 }
	}
	function blockTicketLaabus()
	{
		$jsonVar = $_REQUEST["booking"];
		//echo $jsonVar;
		$onword = json_decode($jsonVar);
		...
		/*try
		{
		$block_key = blockTicket('{"availableTripId":"1000075122980799550","boardingPointId":"35005","destination":"491","inventoryItems":[{"seatName":"L20","ladiesSeat":"false","passenger":{"age":"32","primary":"true","name":"veeru RAJU","title":"Laabus Ticket Booking","gender":"male","idType":"Pan Card","email":"piridi.venkat@gmail.com","idNumber":"123456","address":"Sample Address","mobile":"9177531066"},"fare":250}],"source":"3"}');
		}
		catch(Exception $e)
		{
		var_dump($e);
        ...
        if($this->Booking_model->create_booking_app($bookingKey,$onword)){
			//exit("Reached at the end of debugging...");
			$block_key = blockTicket($jsonVar);	
			//$con=confirmTicket($block_key);
			//var_dump($block_key);die;
			$response["block_key"] =  $block_key;
			$response["unique_key"] =  $bookingKey;
			
		}
		
		//$response["con"] =  $con; 
		header('Content-Type: application/json');
       // echo json_encode(preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', "", $response));
        echo json_encode($response);
        exit;
	}
	function finalconfirmtkt()
	{
		$block_key = $_REQUEST["block_key"];
		$unique_key = $_REQUEST["unique_key"];
		$con = $this->paymentGateway_callback_app($block_key, $unique_key); 
		//$con=confirmTicket($block_key);		
		$response["key"] =  $block_key;
		$response["ticket_no"] =  $con; 
		header('Content-Type: application/json');
       // echo json_encode(preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', "", $response));
       echo json_encode($response);
        exit;
    ...
    public function paymentGateway_callback($transationKey,$payment_status){
	 ...
	 public function paymentGateway_callback_app($blockkey,$unique){
        //echo "111";
        // echo "<br><br>----------------<br><br>";
        //Onword processing 
        $con = $this->payment_process_app($blockkey, $unique, $unique, '');
        return $con;

       
    }

public function payment_process_app($block_ticket,$unique_key,$payment_transationKey='', $payment_status = 'failed'){
            ...
            if($payment_transationKey != "" && $payment_status != 'failed'){   
               ...
               // $con =rand();
                if($con!=''){
                    ...
                    $this->Booking_model->update_ticket_status_app($con,$unique_key,$payment_transationKey,'success');
					return $con;
                   // echo "Success";
                    //echo "<br><br>----------------<br><br>";
                }else{
                ...
                $this->Booking_model->update_ticket_status_app($con,$unique_key,$payment_transationKey,$payment_status);  
                echo "Faile One ";
                //echo "<br><br>----------------<br><br>";
            }
        }else{
