...
class Laabuswebservice extends CI_Controller
{
	...
	public function getBusMarkUpData()
	{
		$commision_amt = $this->users->get_AgentCommisionAmountBySubCat('All Buses');
		$response['commision_amt'] = $commision_amt;
	 ...
	 public function cancelData($tin){
       
       echo  getCancellationData($tin);
        
    }
	
	
	public function cancelbustkt()
	{
		...
		//echo "sasas";die;
	 if(isset($_REQUEST['tin']) && $_REQUEST['tin']!="" && isset($_REQUEST['seatName']) && $_REQUEST['seatName']!=""){
            
            $ticket = array();
            $ticket['tin']= $_REQUEST['tin']; 
            $ticket['seatsToCancel'] = $_REQUEST['seatName'];
            
           $json = json_encode($ticket);
            //echo "sasasa";
           // var_dump($json);
           $data1 =  cancelTicket2($json);
          // var_dump($data1);
          // print_r($data1);
          // echo "<br++++>";
           ...
           $data2 = getCancellationData($_REQUEST['tin']);
           // echo "saassa";
          // var_dump($data2);
           // print_r($data2);
          $this->ticketDeatils->cancelationData($data1);
		  
		  $response["success"] = "Ticket Cancelled";
		  header('Content-Type: application/json');
        echo json_encode($response);
        exit;
        }
    ...
    public function saveKYC()
    {
        ...
        if (isset($_REQUEST['full_name'])) {
            $usersData = array('name' => $_REQUEST['full_name'], 'mobile' => $_REQUEST['mobile_no'], 'email_id' => $_REQUEST['email_id'], 'city_name' => $_REQUEST['city'], 'pincode' => $_REQUEST['postal_code'], 'company_name' => $_REQUEST['company_name']);
            $profileKYCData = array('organization_name' => $_REQUEST['company_name'], 'permanent_address' => $_REQUEST['address']);
            $saveUserInfo = $this->users->update_user_in_profile('users', $usersData, $user_id);
        } else {
            $profileKYCData = array('bussiness_type' => $_REQUEST['business_type'], 'pancard_number' => $_REQUEST['pan_number'], 'resident_proof_number' => $_REQUEST['aadhar_number']);
            $path = 'uploads/profile/' . $user_id;
            if (isset($_REQUEST['photo']) || $_REQUEST['pancard'] || isset($_REQUEST['proof'])) {
                if (!file_exists($path)) {
                    mkdir($path, 0777, true);
                }
            }
            if (isset($_REQUEST['photo'])) {
                file_put_contents($path . '/pan_' . $_REQUEST['pan_number'] . '.jpeg', base64_decode($_REQUEST['pancard']));
            }
            if (isset($_REQUEST['pancard'])) {
                $this->saveBinaryImage($_REQUEST['pancard'], $path.'/pancard.'.$_REQUEST['pancard_extension']);
		$profileKYCData['pancard'] = 'pancard.'.$_REQUEST['pancard_extension'];
		$data['pan_path'] = $path.'/pancard.'.$_REQUEST['pancard_extension'];
            }
            if (isset($_REQUEST['proof'])) {
                $this->saveBinaryImage($_REQUEST['proof'], $path.'/proof.'.$_REQUEST['proof_extension']);
		$profileKYCData['resident_proof'] = 'proof.'.$_REQUEST['proof_extension'];
		$data['aadhar_path'] = $path.'/proof.'.$_REQUEST['proof_extension'];
            }
        }

        $checkKYCExistance = $this->users->get_userprofile_info('profile_kyc', $_POST['user_id']);
        if ($checkKYCExistance) {
            $updateKYC = $this->users->update_user_in_profile('profile_kyc', $profileKYCData, $user_id);
        } else {
            $profileKYCData['user_id'] = $user_id;
            $updateKYC = $this->users->insert_user_in_profile('profile_kyc', $profileKYCData);
        }

        $response['status'] = "success";
        $response['code'] = "200";
        $response['message'] = "KYC saved successfully";
        $response['data'] = $data;
        header('Content-Type: application/json');
        echo json_encode($response);
        exit;
    ...
    public function wallet_list()
    {
        ...
        $walletlist['Lists'] = $this->users->get_wallet_list($user_id);
        //print("<pre>");
        //print_r($walletlist);
        header('Content-Type: application/json');
    ...
    public function Orders()
    {
        ...
        if (isset($_REQUEST["search"])) {
            ...
            if (!empty($_REQUEST["category"])) {
                $search_params["cat_id"] = $_REQUEST["category"];
            }
        ...
        $order_details['Reports'] = $this->Va_Commisions_model->order_details($search_params);
	...
	{
		$user_id = $_REQUEST["user_id"];
		$order_details['Reports'] = $this->ticket->get_ticketDetials($user_id);
	...
	public function getOffersplan(){		
		$plan = $this->users->getOffersplan($_REQUEST['operator'],$_REQUEST['amount']);
			//print_r($plan);
			if(!empty($plan))
				$data['plan'] =  $plan[0]->validity;
			else
				$data['plan'] = "General Plan";
    ...
    public function add_funds()
    {
        ...
        $reference_number = $_REQUEST['ref_no'];
        $ttype = $_REQUEST['transfer_type']; //1
        $ptype = $_REQUEST['ptype']; //1
        ...
        $dataWalletOffers1 = $this->users->getwalletoffersWalletDetailsPromoCode($role_id, $promo_code, $amount, 'exact');
        $dataWalletOffers2 = $this->users->getwalletoffersWalletDetailsPromoCode($role_id, $promo_code, $amount, 'upto');
        ...
        $arrayData = array(
            'user_id' => $user_id,
            'total_amount' => $amount,
            'information' => json_encode(array('user_id' => $user_id, 'total_amount' => $amount, 'transaction_status_id' => 1, 'shipping_method_id' => 1, 'transaction_stage_time' => date("Y-m-d H:i:s"))),
        ...
        if ($_REQUEST['ptype'] == 1) {
            ...
            if (!empty($dataJoinOffers)) {
                ...
                if ($amount >= $dataJoinOffers[0]->min_wallet_amoun) {
                    ...
                    $totamount = $amount + $dataJoinOffers[0]->offer_amount;
                ...
                if ($dataWalletOffers1[0]->offer_mode == "INR") {
                    $totamount = $amount + $dataWalletOffers1[0]->add_amount;
                    ...
                    $extra_amt = $amount * $dataWalletOffers1[0]->add_amount / 100;
                    $totamount = $amount + $extra_amt;
                ...
                if ($dataWalletOffers2[0]->offer_mode == "INR") {
                    $totamount = $amount + $dataWalletOffers2[0]->add_amount;
                    $updates_status = $this->users->update_wallet_offer_min_wallet($user_id, $promo_code, $dataWalletOffers2[0]->add_amount, 1);
                } else if ($dataWalletOffers1[0]->offer_mode == "PEC") {
                    $extra_amt = $amount * $dataWalletOffers2[0]->add_amount / 100;
                    $totamount = $amount + $extra_amt;
                    $updates_status = $this->users->update_wallet_offer_min_wallet($user_id, $promo_code, $extra_amt, 1);
                } else {
            ...
            if ($ttype == 1) {
                $arrayData['account_number'] = $_REQUEST['account_number'];
            } else {
                ...
                $arrayData['counter_file'] = 'Mobile Uploads';
            }
...
//---------------------------------
    ...
    {
        $data['state'] = $this->users->getstatearray('IN');
        header('Content-Type: application/json');
        echo json_encode($data);
        exit;
    }
    ...
    {
        $data['district'] = $this->users->getdistrict($_REQUEST['state_code']);
        header('Content-Type: application/json');
        echo json_encode($data);
        exit;
    }

    public function ajax_cities_list()
    {
        $data['cities'] = $this->users->getcities($_REQUEST['district']);
        header('Content-Type: application/json');
        echo json_encode($data);
        exit;
    }
	...
	$usertype = $_REQUEST["usertype"];
        ...
        $city = $_REQUEST["city"];
        $chp_id = isset($_REQUEST['chp_id']) ? $_REQUEST['chp_id'] : 0;
        $emailcount = $this->users->email_exists($usertype, $email);
        if ($emailcount->cnt == 0) {
            // return true;
            if ($usertype == 2) {
            ...
            if ($usertype == 6) {
                $user_id = $this->users->create_agent($chp_id);
		if($user_id !=''){
			$userArray = $this->users->checkUserTypeLaabus($user_id);
    ...
    public function Login()
    {
		...
		'agent_subscription_amt' => $row->agent_subscription_amt,
		'subscription_wallet_amt' => $row->subscription_wallet_amt,
            );
        } else {
            $data['LoginResponse'] = array(
                'status' => 'fail',
    ...
    public function dashboard()
    {
        ...
        if (!check_login_status()) {
            ...
            if (!$this->input->is_ajax_request()) {
                ...
                //echo "there";
                $this->load->view('website/agent/dashboard', $data);
            }
        }
    }
    ...
    public function wallet_listSessi()
    {
        $data['category'] = $this->Cat->get_category();
        $data['roles'] = $this->users->get_roles();
        if (!check_login_status()) {
...
//---------------------------------
    ...
    public function add_funds_sessoon()
    {
        // print_r($this->session->userdata());
        $data['category'] = $this->Cat->get_category();
        $data['roles'] = $this->users->get_roles();
