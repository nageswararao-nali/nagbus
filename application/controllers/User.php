...
class User extends CI_Controller {
	...
	public function printTicket($tin){
		...
		$commision_amt = $this->users->get_AgentCommisionAmountBySubCat('All Buses');
			...
			if( $this->session->userdata('role_id') == 6 )
			{
				...
				{
					$ticket123 = json_decode($ticket, true);
					//print_r($ticket123);
					//print_r($commision_amt);
					$markup = $ticket123["inventoryItems"]["fare"]*$commision_amt[0]->mark_comm_value/100;
				}
			}
        ...
        $data['ticket'] = $ticket;
		$data['agent_comm'] = $markup;
		
       $this->load->view('website_template/header', $data);
	   $this->load->view('website/user/print_ticket',$data1);
	   $this->load->view('website_template/footer');
    }
    
    
    public function PassengerList($bookingKey,$journey_date=''){     
        
        if(isset($_POST['tin']) && $_POST['tin']!="" && isset($_POST['seatName']) && $_POST['seatName']!=""){
            
            $ticket = array();
            $ticket['tin']= $_POST['tin']; 
            $ticket['seatsToCancel'] = $_POST['seatName'];
            
           $json = json_encode($ticket);
            
           $data1 =  cancelTicket($json);
         
           $data2 = getCancellationData($_POST['tin']);
           
       echo  $this->ticketDeatils->cancelationData($data1,$data2);
        }
        
        $data['category'] = $this->Cat->get_category();
		$data['roles'] = $this->users->get_roles();   
        $data1['passengers']=$this->ticket->get_passengerList($bookingKey);
		$data['journey_date'] = $journey_date;		
        
       $this->load->view('website_template/header', $data);
	   $this->load->view('website/user/passenger_list',$data1);
	   $this->load->view('website_template/footer');
    }
	...
	public function offers() {
		...
		$data1['offers'] = $this->users->getoffersusers($role_id);
...
$tot_W_a = $wallet_amount+$totamount;
  ...
  $message = 'Dear '.$name.', Your Wallet is filled with INR '.$totamount.'. Your net Wallet Amount is '.$tot_W_a.'. thank you for using w LAABUS . download  app @ https://goo.gl/QWUiJ';
	...
	public function Orders() {
		...
		if( $role_id == 6 )
		{
			redirect('agent/Orders#menu1');die;
		}
		$user_id=$this->session->userdata('user_id');
		...
		$data1['ticket_details'] = $this->ticket->get_ticketDetials($user_id);
			//print_r($data1['ticket_details']);exit;
		 $this->load->view('website_template/header', $data);
