...
class Users_Model extends CI_Model {
	...
	public function getOffersplan($operator,$amount){
		...
		else if($operator == 3 || $operator == 301 || $operator == 302 || $operator == 303 )
		{
			$operator = 4;
		}
		else
...
visit www.laabus.com for exiting offers.';
	...
	//var_dump($return_val);exit;
        ...
        public function get_agent_today_earning($user_id)
		{
			...
			//echo $date = '2017-04-08';
			//echo "SELECT SUM(agent_comm+agent_ref_comm) as earnings FROM `transaction` WHERE   transaction_status = 1 AND (`end_user_id`= $user_id OR `agent_id`= $user_id ) and date(order_date) = '$date' ";
			$date = date('Y-m-d');
