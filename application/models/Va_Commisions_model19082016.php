...
class Va_Commisions_model extends CI_Model {
	...
	public function updatesPaymentStatus($sales_ids,$status)
	{
		$sql = "UPDATE  transaction SET  mark_as_credit_user ='".$status."' where sales_id IN (".$sales_ids.") ";
	...
	public function get_comm_categorywise_details($condition = NULL) {
	 ...
	 public function order_details($condition='')
	 {
		...
		$qry  .= " where a.transaction_status = 1  ";
	  ...
	  public function order_details_credit($condition='')
	 {
		$qry = "SELECT a.*,b.*,c.*,d.name as agentname,e.name as chnlname,f.name as smdname,sales.user_id as recharge_by,sales.ordertouser as ordertouser FROM `transaction` a
		  LEFT JOIN  categories b ON a.cat_id = b.cat_id ";
		...
		$qry  .= " where a.transaction_status = 1 AND a.mark_as_credit_user != 0 ";
		   ...
		   if(!empty($condition["agent_id"]))
		 {
		   ...
		   if(!empty($condition["mark_as_credit_user"]))
		 {
			$qry  .=  " AND a.mark_as_credit_user = '".$condition["mark_as_credit_user"]."' ";
		 }
		  if(!empty($condition["cnl_part"]))
		 {
			$qry  .=  " AND e.user_id = '".$condition["cnl_part"]."'  ";
	  ...
	  public function order_details_user($condition='')
	 {
	   ...
	   public function order_details_user_new($condition='')
	 {
		$qry = "SELECT a.*,b.*,c.*,d.name as agentname FROM `transaction` a
		  INNER JOIN users d  ON a.end_user_id=d.user_id AND d.user_id = '".$this->session->userdata('user_id')."'  LEFT JOIN    categories b ON a.cat_id = b.cat_id ";
