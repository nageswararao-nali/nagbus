...
class Salemodel extends Parent_model{
	...
	function create_order($a){
		...
		//
		if($_REQUEST["category_id"] == "9999") //added on 18122016 by Venkat Piridi
		{
			$sub_cats[0]->sub_cat_id = $_REQUEST['product_id'];
			$name = $_REQUEST["name"];
		...
		//end commision data
			...
			$operator=$this->session->userdata('operator');
			if( $a['recharge_type'] == "DTH" || $a['recharge_type'] == "Data Card"  ||  $a['recharge_type'] == "Mobile postpaid" ||  $a['recharge_type'] == "Electricity" ||  $a['recharge_type'] == "Landline" )
			{
		...
		if($_REQUEST["category_id"] == "9999") //added on 18122016 by Venkat Piridi
		{
			$res = $this->db->query("SELECT * FROM `va_commissions_all` WHERE `subcategory_id` = '".$_REQUEST["sub_cat_id"]."' AND category_id  = '".$_REQUEST["category_id"]."'");
		}
		else
		{
		...
		if($_REQUEST["category_id"] == "9999" || $_REQUEST["category_id"] == "8888" ) //added on 18122016 by Venkat Piridi
		{
			$TotAmount=$_REQUEST["Amount"];
		}
		...
		//print_r("<pre>");
		//print_r($this->session->all_userdata());die;
		if( $this->session->userdata("our_comm_type") == "PEC" )
		{
			$total_comm = $this->session->userdata("our_comm_value")*$this->session->userdata("rcAmount")/100;
		}
		else
		{
			$total_comm = $this->session->userdata("our_comm_value");
		}
		...
		/*echo "ID:".$channel_part_id;
		echo "<br>";
		echo "Channel Comm:".$chnl_comm;
		echo "<br>";
		echo "laabus_comm:".$laabus_comm;
		echo "<br>";
		print_r($this->session->userdata());
		die;*/
		...
		$commisions_data = array("cat_id"=>$q[0]->category_id,
								...
								"mark_as_credit_comments"=>$this->session->userdata('mark_credit_text'),
								"mark_as_credit_user"=>1,
