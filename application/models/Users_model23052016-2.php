...
/*****************************************************************************************************************/
	...
	public function mobile_exists($role_id,$mobile) {
		...
		{
			$date = 'Y-m-d';
			$result = $this->db->query("SELECT SUM(agent_comm+agent_ref_comm) as earnings FROM `transaction` WHERE `end_user_id`= $user_id OR `agent_id`= $user_id and date(order_date)= '$date' " )->result_array();
            //echo $this->db->last_query();exit;
            return $result;
		}
		...
		{
			$date = 'Y-m-d';
			$result = $this->db->query("SELECT SUM(agent_comm+agent_ref_comm) as earnings FROM `transaction` WHERE `end_user_id`= $user_id OR `agent_id`= $user_id and date(order_date) BETWEEN '2016-01-01' AND '2016-12-31'  " )->result_array();
            //echo $this->db->last_query();exit;
            return $result;
		}
		...
		{
			$date = 'Y-m-d';
			$result = $this->db->query("SELECT SUM(agent_comm+agent_ref_comm) as earnings FROM `transaction` WHERE `end_user_id`= $user_id OR `agent_id`= $user_id and date(order_date) BETWEEN '2016-05-01' AND '2016-05-31'  " )->result_array();
            //echo $this->db->last_query();exit;
            return $result;
		}
